...
var Protocol = function(socket, data) {
	...
	this._cable.on('message', function(message, cb) {
		...
		var emit = function() {
			self.emit.apply(self, arguments) || cb(new Error('unsupported opcode'));
		};

		switch (opcode) {
			case 0:  return emit('get', id, stringify(cb));
			case 1:  return emit('put', id, JSON.parse(payload.toString()), cb);
			case 2:  return emit('update', id, JSON.parse(payload.toString()), cb);
			case 3:  return emit('del', id, cb);
			case 4:  return emit('list', cb);
			case 5:  return emit('ps', cb);
			case 6:  return emit('start', id, cb);
			case 7:  return emit('stop', id, cb);
			case 8:  return emit('restart', id, cb);
			case 9:  return emit('distribute', id, cb);
			case 10: return emit('interested', id);
			case 11: return emit('uninterested', id);
			case 12: return emit('stdout', id, payload);
			case 13: return emit('stderr', id, payload);
			case 14: return emit('spawn', id);
			case 15: return emit('exit', id);
		}

		cb(new Error('unsupported opcode'));
	});
...
Protocol.prototype.spawn = function(id) {
	this._send(14, id, null);
};

Protocol.prototype.exit = function(id) {
	this._send(15, id, null);
};
