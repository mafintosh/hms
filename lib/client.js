...
module.exports = function (remote, opts) {
  ...
  var writeLog = function (id, message) {
    if (!logs[id]) return
    logs[id].forEach(function (rs) {
      rs._push(xtend(message))
    })
  }

  var onlog = function (message) {
    writeLog(message.id, message)
    writeLog('*', message)
  }
  ...
  var connect = thunky(function loop (cb) {
    ...
    req.on('connect', function (res, socket, data) {
      ...
      p.handshake({
        ...
        if (!handshake.type) return cb(new Error('Server sent invalid handshake'))
        cb(null, p, socket)
      })
    })
  ...
  client.log = function (id, format) {
    if (typeof id === 'function') return client.log(null, id)
    if (!id) id = '*'

    var pt = new stream.Readable({objectMode: true})

    logs[id] = logs[id] || []
    logs[id].push(pt)
    if (logs[id].length === 1) client.subscribe(id)

    pt._read = noop
    pt._push = function (message) {
      pt.push(format ? format(message) : message)
    }

    pt.destroy = function () {
      if (!logs[id]) return
      var i = logs[id].indexOf(pt)
      if (i === -1) return
      logs[id].splice(i, 1)
      if (logs[id].length) return
      delete logs[id]
